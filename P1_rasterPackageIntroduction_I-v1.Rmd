...
handling, manipulation and modelling in R. Examples will be given along with the tutorials. 
Some exercises, with different difficult levels, will be provided so you can practice.
...
First we need to install the `raster` package (and `sp`): 
...
Next, download and unzip sample data. We will use 
...
Then use `print` to inspect the _"essential"_ attributes of the dataset.
...
with a single layer. We can also see the dimension: __579__ rows, __555__ columns and the pixel size 
in x and y dimensions, or the __spatial resolution__, equal to 80m (we are using a projected coordinate 
...
is actually "made of" metadata and a link to the actual raster data on disk.         
...
In raster package, a __proj4string__ is used to set and define the CRS of the data. This string contains 
...
summarising and accessing values at speficic locations.
...
# Generate the density plot object and then draw it
ds <- density(rst, plot = FALSE)
lines(ds, col = "red", lwd = 2)

```

![](./img/P1_raster_hist-1.png)

Calculating summary statistics is fairly easy using the raster package. The generic method `summary` is 
available for this type of object (note: this function will use a sample of the entire to calculate 
statistics).
...
function (no sample employed either).
...
for very large Raster* objects except for certain predefined functions: sum, mean, min, max, sd, 'skew' 
and 'rms'.     
...
Extracting raster values using points as input:     
...
Typically, we are also interested in extracting raster values for specic regions-of-interest (ROI). In this 
example we will use a polygon (a broadleaf forest area) to assess the distribution of elevation values 
within it. 
